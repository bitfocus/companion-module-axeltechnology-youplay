...
class PlayerInfo 
{
	...
	async PlayerDataStatus(instance) 
	{
		...
		if(data==null)
		{
			//here when an instance is not avaiable we fill it with the initialized data
			this.Instance = instance;
			this.OnAirUniqueID=0;
			this.OnAirRemain = 0;
			this.PlayerStatus=0;
			this.PlayerLoop=false;
			this.PlayerOneAtATime=false;
			this.PlayerStartToEnd=false;
			this.OnAirDuration=0;
			this.OnAirPosition = 0;
			this.PlayerMixerEnabled = false;
			this.PlayerLogoEnabled =false;
			this.PlayerAudioPreviewEnabled = false;
		}
		else
		{
			this.Instance = instance;
