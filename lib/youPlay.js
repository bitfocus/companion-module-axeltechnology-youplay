...
class YouPlay 
{
	//the constructor uses the Ip and url port specified by the user 
	constructor(youPlayIp, urlPort, LogManager) 
	{
		this.youPlayIp = youPlayIp;
		this.urlPort = urlPort;
		this.LogManager = LogManager;

		if (youPlayIp !== null && youPlayIp !== undefined && urlPort !== null && urlPort !== undefined && youPlayIp !== '' && urlPort !== '') 
		{
			...
			this.LogManager('warn', 'Incomplete configuration: IP or Port are missing');
		}  
	}
	//------------------------------------------------------------------------------------------------
	...
	async Connect(YPinstance) {
		...
		const url = this.getUrl(YPinstance, 'GetApplicationInfo', 'data');
		this.LogManager('info', 'Connect YouPlay ' + YPinstance + " " + url);
		...
		try {
			...
			this.LogManager('info', 'YouPlay response');
	
			if (response.status === 200) {
				this.LogManager('info', 'Successful YouPlay connection');
				return true;
			} else {
				this.LogManager('warn', 'Rejected YouPlay connection');
				return false;
			}
		} catch (err) {
	...
	{
		//console.log("getPlayerStatus" + YPinstance);
		await this.loadFetch()
		try {
		...
		{
			//this.LogManager('warn',` YouPlay ${YPinstance} isnt available`);
